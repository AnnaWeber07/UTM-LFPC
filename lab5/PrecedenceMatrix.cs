...
namespace CoreParser
{
    public class PrecedenceMatrix
    {
        ...
        public MatrixFiller M { get; set; }

        public PrecedenceMatrix(RelationsTable firstLastTable, Grammar grammar)
        {
            Rule1 = CompleteRule1(grammar);
            Rule2 = CompleteRule2(firstLastTable, grammar);
            Rule3 = CompleteRule3(firstLastTable, grammar);
            M = new MatrixFiller(grammar.Vn, grammar.Vt);
            FillMatrix();
        }
        ...
        private static Dictionary<string, List<string>> CompleteRule2(RelationsTable firstLastTable, Grammar grammar)
        {
            var result = new Dictionary<string, List<string>>();
            foreach (var values in grammar.P.Values)
            {
        ...
        private static List<Tuple<List<string>, List<string>>> CompleteRule3(RelationsTable firstLastTable, Grammar grammar)
        {
